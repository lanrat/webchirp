...
- Fixed the FTDI-over-WebUSB read deadlock (found via the Android loopback stats: exactly 2 transfers, then silence forever): the stream `pull()` resolved without enqueuing when a packet carried only the FTDI 2-byte status header, and per the Streams spec such a pull is never re-invoked — so all reads wedged after the first idle packet. `pull()` now keeps polling through status-only packets (and stalls) and resolves only after enqueuing real payload. This bug broke RX on every platform and was the cause of the Android clone-handshake failure.
- Confirmed working on Android Chrome with an FT231X cable: the loopback test echoes all bytes back and downloading a Baofeng UV-5R over WebUSB completes. This resolves the "known limitation" noted on 2026-06-23.

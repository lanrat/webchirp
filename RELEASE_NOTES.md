...
- Added a WebUSB serial fallback so browsers that expose WebUSB but not Web Serial (e.g. Chrome on Android) can connect to radios. A single device chooser dispatches the selected adapter to a chip-specific driver: a built-in FTDI-over-WebUSB driver for FTDI adapters (FT231X/FT232R, etc.), and Google's `web-serial-polyfill` for USB CDC-ACM devices. Other vendor-specific cables (CH340/CP2102/PL2303) are not supported over WebUSB yet and still require native Web Serial. Native Web Serial is always preferred when present.
